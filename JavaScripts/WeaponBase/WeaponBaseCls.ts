...
export default class WeaponDriver extends Core.Script {
	/** */
	public config: GameDef.WeaponConfig
					...
					hitResult.forEach(e => {
						...
						if (e instanceof Gameplay.HitResult) {
							if (e.gameObject instanceof Gameplay.Character ||
								...
								PrefabEvent.PrefabEvtFight.hit(this.chara.guid, e.gameObject.guid, this.damage, e.impactPoint.clone())
								return
							}
						}

						if (e instanceof Gameplay.Character || e instanceof Core.GameObject) {
							PrefabEvent.PrefabEvtFight.hit(this.chara.guid, e.guid, this.damage, e.worldLocation.clone())
							return
						}
	...
	protected onUpdate(dt: number): void {
		...
		switch (this.weaponObj.getCurrentState()) {
			...
			case Gameplay.HotWeaponState.Firing:
				if (this.isEmptyToDestroy && this.totalAmmo == 0 && this.weaponObj.fireComponent.currentBulletSize == 0) {
					this.unEquip()
				}
				break
		...
		if (this.weaponUI) {
			this.weaponUI.changeBullet(this.weaponObj.fireComponent.currentBulletSize, this.totalAmmo)
			if (this.keepTime != -1) {
				this._restTime -= dt
	...
	protected hit(hitResult: Core.GameObject[] | Gameplay.HitResult[]) {
		...
		if (this.detectRadius > 10) { // 矩形检测结果
			...
			if (this.hurtRadius > 10) {
				let sphereResult = Gameplay.sphereOverlap((hitResult[0] as Core.GameObject).worldLocation, this.hurtRadius, DEBUG_FLAG)
			...
			if (this.hurtRadius > 10) {
				let sphereResult = Gameplay.sphereOverlap((hitResult[0] as Gameplay.HitResult).impactPoint, this.hurtRadius, DEBUG_FLAG)
				this.clientOnHit(sphereResult, this.player.getPlayerID(), true)
			} else {
...
private serverEquip(playerID: number): void {
	...
	this.weaponObj.equipment(player.character, this.equipmentSlot)
	this.isEquiped = true
...
private clientEquip(pickPlayer: Gameplay.Player, gender: number): void {
	...
	this.weaponObj.equipment(this.chara, this.equipmentSlot)
	...
	this.camera.targetArmLength = 400
	this.camera.cameraFOV = this.equipmentCameraFov
	this.camera.cameraRelativeTransform = new Type.Transform(this.equipmentCameraOffset, this.camera.cameraRelativeTransform.rotation, this.camera.cameraRelativeTransform.scale)
	...
	this.weaponUI = UI.UIManager.instance.show(WeaponUI, this, this.weaponObj.accuracyOfFireEnable ? this.weaponObj.accuracyOfFireComponent.defaultDispersionHalfAngle : 0, this.WaponIcon, this.weaponName)
...
private clientInitAmmoEntityRoot(): void {
	...
	this.ammoPool = new SimpleObjectPool<Core.GameObject>(this.instanceAmmo.bind(this), (obj: Core.GameObject) => { obj.destroy() }, (obj: Core.GameObject) => { obj.setVisibility(Type.PropertyStatus.Off) })
}

/* 客户端初始化弹壳实体 */
private clientInitCasingEntity(): void {
	this.casingEntity = this.weaponObj.getChildByName("casingEntity") as Core.GameObject
	this.casingPool = new SimpleObjectPool<Core.GameObject>(this.instanceCasing.bind(this), (obj: Core.GameObject) => { obj.destroy() }, (obj: Core.GameObject) => { obj.setVisibility(Type.PropertyStatus.Off) })
}
...
private clientInitHitCharaEffect(): void {
	...
	this.hitCharaEffectPool = new SimpleObjectPool<Gameplay.Particle>(this.instanceHitCharaEffect.bind(this), (particle: Gameplay.Particle) => { particle.destroy() }, (particle: Gameplay.Particle) => { particle.detachFromGameObject() particle.stop() })
}

/* 客户端初始化击中物体特效 */
private clientInitHitEffect(): void {
	this.hitEffect = this.weaponObj.getChildByName("hitEffect") as Gameplay.Particle
	this.hitEffectPool = new SimpleObjectPool<Gameplay.Particle>(this.instanceHitEffect.bind(this), (particle: Gameplay.Particle) => { particle.destroy() }, (particle: Gameplay.Particle) => { particle.detachFromGameObject() particle.stop() })
}
...
private clientInitHitCharaSound(): void {
	...
	this.hitCharaSoundPool = new SimpleObjectPool<Gameplay.Sound>(this.instanceHitCharaSound.bind(this), (sound: Gameplay.Sound) => { sound.destroy() }, (sound: Gameplay.Sound) => { sound.stop() })
...
private clientInitHitSound(): void {
	...
	this.hitSoundPool = new SimpleObjectPool<Gameplay.Sound>(this.instanceHitSound.bind(this), (sound: Gameplay.Sound) => { sound.destroy() }, (sound: Gameplay.Sound) => { sound.stop() })
}
	...
	if (this.weaponObj.getCurrentOwner() == this.chara) {
		...
		if (this.ammoEntityRoot.getChildren().length > 0) {
			...
			for (let i = 0 i < this.weaponObj.fireComponent.currentMultipleShot i++) {
				...
				let endLoc = cameraShootDir.multiply(SHOOT_RANGE).add(this.camera.cameraWorldTransform.location)
				let shootDir = endLoc.clone().subtract(this.ammoEntityRoot.worldLocation)
				let hitRes = Gameplay.lineTrace(this.camera.cameraWorldTransform.location, endLoc, true, DEBUG_FLAG)
				...
				if (this.ammoSpeed < MAX_SHOOTSPEED || this.isBlock) { // 如果弹药速度小于最大飞行速度值或者弹道有明显阻挡情况下，弹药走真实弹道
					...
					this.ammoArray.push(new Ammo(this.chara, this.ammoPool, this.ammoEntityRoot.worldLocation, ammoDirection, this.shootRange, this.ammoSpeed, this.gravityScale, this.detectRadius))
					...
					if (hitRes.length > 0) { // 屏幕中心射线击中
						this.ammoArray.push(new Ammo(this.chara, this.ammoPool, this.ammoEntityRoot.worldLocation, ammoDirection, shootDir.length, this.ammoSpeed, this.gravityScale, 0, hitRes))
					} else { // 屏幕中心射线未击中
						this.ammoArray.push(new Ammo(this.chara, this.ammoPool, this.ammoEntityRoot.worldLocation, ammoDirection, shootDir.length, this.ammoSpeed, this.gravityScale, 0))
					}
			...
			let end = ammoDirection.clone().multiply(this.shootRange).add(this.ammoEntityRoot.worldLocation)
			if (this.detectRadius < 10) {
				let lineResult = Gameplay.lineTrace(this.ammoEntityRoot.worldLocation, end, true, DEBUG_FLAG)
				lineResult = lineResult.filter(e => {
					return !(e.gameObject instanceof Gameplay.Trigger)
				})
				this.hit(lineResult)
			} else {
				let boxResult = Gameplay.boxOverlapInLevel(this.ammoEntityRoot.worldLocation, end, this.detectRadius, this.detectRadius, DEBUG_FLAG)
				this.hit(boxResult)
			}
		}
	}
...
private updateBlockFire(): boolean {
	...
	let lineResultMuzzle = Gameplay.lineTrace(this.ammoEntityRoot.worldLocation,
		this.ammoEntityRoot.getForwardVector().multiply(this.fireBlockDistance).add(this.ammoEntityRoot.worldLocation),
		true, DEBUG_FLAG)
...
private cameraUpdate(dt: number) {
	...
	if (this.isAimming) {
		this.camera.cameraFOV -= dt * this.aimSpeed
		if (this.camera.cameraFOV < this.aimCameraFov) {
			this.camera.cameraFOV = this.aimCameraFov
			this.isZooming = false
		}
	} else {
		this.camera.cameraFOV += dt * this.aimSpeed
		if (this.camera.cameraFOV > this.equipmentCameraFov) {
			this.camera.cameraFOV = this.equipmentCameraFov
			this.isZooming = false
		}
	}
